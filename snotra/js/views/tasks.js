...
export function renderTasks(el) {
  ...
  const scheduled = all.filter(t => !t.inbox && t.scheduled && t.status !== 'done')
    .sort((a, b) => a.scheduled.localeCompare(b.scheduled) || store.prioRank(a) - store.prioRank(b));
  const backlog = all.filter(t => !t.inbox && !t.scheduled && t.status !== 'done')
    .sort((a, b) => store.prioRank(a) - store.prioRank(b));
  const done = all.filter(t => t.status === 'done').sort((a, b) => (b.completedAt || 0) - (a.completedAt || 0));
  ...
  ${mode === 'list' ? listHtml(inbox, scheduled, backlog, done) : kanbanHtml(all)}`;
...
function listHtml(inbox, scheduled, backlog, done) {
  return `<div class="grid cols-2">
    ...
    <div>
      <div class="card" style="margin-bottom:14px">
        ...
        ${scheduled.length ? groupByDate(scheduled) : '<div class="empty">No scheduled tasks.</div>'}
      </div>
      <div class="card">
